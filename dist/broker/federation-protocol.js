import { FEDERATION_CAPABILITY_MAX_LENGTH, FEDERATION_CAPABILITY_MIN_LENGTH, FEDERATION_CORRELATION_ID_MAX_LENGTH, FEDERATION_DISPLAY_LABEL_MAX_LENGTH, FEDERATION_IDENTITY_FEATURE, FEDERATION_LOCAL_SCOPE_ID_MAX_LENGTH, FEDERATION_MAX_FEATURES, FEDERATION_MAX_SCOPE_MAPPINGS, FEDERATION_ORIGIN_ID_MAX_LENGTH, FEDERATION_PROTOCOL_NAME, FEDERATION_PROTOCOL_VERSION, FEDERATION_SCOPE_ALIAS_MAX_LENGTH, FEDERATION_SESSION_ID_MAX_LENGTH, FEDERATION_SINGLE_HOP_FEATURE, } from "./federation-types.js";
...
function isSafeText(value, maxLength, requireTrimmed = true) {
...
export function isFederationLoopbackEndpoint(value) {
...
export function isFederationScopeMapping(value) {
    return isRecord(value)
...
export function isFederationScopeBinding(value) {
    return isRecord(value)
...
function hasUniqueMappingAliases(mappings) {
...
function isScopeMappings(value) {
...
function isScopeBindings(value) {
...
function isFeatures(value) {
    if (!Array.isArray(value) || value.length < 1 || value.length > FEDERATION_MAX_FEATURES)
        return false;
    if (!value.every((feature) => typeof feature === "string" && feature.length <= 64 && FEATURE.test(feature)))
        return false;
    if (new Set(value).size !== value.length)
        return false;
    return value.includes(FEDERATION_IDENTITY_FEATURE) && value.includes(FEDERATION_SINGLE_HOP_FEATURE);
}
export function scopeMappingsFromBindings(bindings) {
    return bindings.map(({ localScopeAlias, remoteScopeAlias }) => ({ localScopeAlias, remoteScopeAlias }));
}
export function invertScopeMappings(mappings) {
    return mappings.map(({ localScopeAlias, remoteScopeAlias }) => ({
        localScopeAlias: remoteScopeAlias,
        remoteScopeAlias: localScopeAlias,
    }));
}
export function bindingsMatchPeerMappings(bindings, peerMappings) {
...
export function isBrokerDialPeerRequest(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ["type", "requestId", "endpoint", "capability", "localOrigin", "remoteOrigin", "scopeBindings"], ["stateId"]))
        return false;
...
function isSuppliedPeerRequest(value, type) {
    if (!isRecord(value) || !hasOnlyKeys(value, ["type", "requestId", "linkId", "localOrigin", "remoteOrigin", "scopeBindings"], ["stateId"]))
        return false;
...
export function isBrokerAcceptPeerRequest(value) {
    return isSuppliedPeerRequest(value, "broker_accept_peer");
}
export function isBrokerStartPeerRequest(value) {
    return isSuppliedPeerRequest(value, "broker_start_peer");
}
export function isBrokerListScopesRequest(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ["type", "requestId"], ["stateId"]))
        return false;
    return value.type === "broker_list_scopes"
        && isFederationCorrelationId(value.requestId)
        && (value.stateId === undefined || isFederationCorrelationId(value.stateId));
}
export function isBrokerListScopesResult(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ["type", "requestId", "ok"], ["localOrigin", "scopes", "code", "error"]))
        return false;
    ...
    if (value.ok === true) {
        if (!isRecord(value.localOrigin)
            ...
            || !isCanonicalFederationOriginId(value.localOrigin.id))
            return false;
        if (!Array.isArray(value.scopes) || value.scopes.length > 64)
            return false;
        const seen = new Set();
        for (const scope of value.scopes) {
            if (!isRecord(scope) || !hasOnlyKeys(scope, ["scopeId", "liveSessions"]))
                return false;
            const scopeId = scope.scopeId;
            if (scopeId !== null && (typeof scopeId !== "string"
                || scopeId.length === 0
                || scopeId.length > 256
                || /[\p{Cc}\p{Cf}]/u.test(scopeId)))
                return false;
            if (!Number.isSafeInteger(scope.liveSessions) || scope.liveSessions < 0)
                return false;
            if (seen.has(scopeId))
                return false;
            seen.add(scopeId);
...
export function isFederationBridgeAttach(value) {
    return isRecord(value)
...
export function isPeerHello(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ["type", "protocol", "version", "linkId", "origin", "expectedPeerOrigin", "scopeMappings", "features"]))
        return false;
...
export function isPeerHelloAck(value) {
...
function isControlResult(value, type) {
    if (!isRecord(value) || value.type !== type || !isFederationCorrelationId(value.requestId) || typeof value.ok !== "boolean")
        return false;
    if (value.ok) {
...
export function isBrokerDialPeerResult(value) {
    return isControlResult(value, "broker_dial_peer_result");
}
export function isBrokerAcceptPeerResult(value) {
    return isControlResult(value, "broker_accept_peer_result");
}
export function isBrokerStartPeerResult(value) {
    return isControlResult(value, "broker_start_peer_result");
}
function isStableSessionId(value) {
    return isSafeText(value, FEDERATION_SESSION_ID_MAX_LENGTH, false);
}
function isOriginQualifiedSessionIdentity(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ["originId", "remoteScopeAlias", "remoteStableSessionId"]))
        return false;
    return isCanonicalFederationOriginId(value.originId)
        && isCanonicalFederationScopeAlias(value.remoteScopeAlias)
        && isStableSessionId(value.remoteStableSessionId);
}
export function encodeOriginQualifiedSessionIdentity(identity) {
    if (!isOriginQualifiedSessionIdentity(identity))
        throw new Error("Invalid origin-qualified session identity");
...
export function decodeOriginQualifiedSessionIdentity(value) {
    if (!value.startsWith(QUALIFIED_ID_PREFIX) || value.length > MAX_QUALIFIED_ID_LENGTH)
        return undefined;
